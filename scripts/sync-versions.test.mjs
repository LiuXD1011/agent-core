...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	...
	try {
		await writeManifest(root, "packages/ai", {
			name: "@liuxuedeng/pi-core-ai",
			version: "2.0.0",
		});
		await writeManifest(root, "packages/coding-agent", {
			name: "@liuxuedeng/pi-core",
			version: "2.0.0",
		});
		await writeManifest(root, "packages/evals", {
			name: "@liuxuedeng/pi-core-evals",
			version: "9.9.9",
			private: true,
			dependencies: {
				"@liuxuedeng/pi-core": "^1.0.0",
				"@mariozechner/pi-ai": "npm:@liuxuedeng/pi-core-ai@1.0.0",
			},
		});
		...
		assert.equal(evalsManifest.dependencies["@liuxuedeng/pi-core"], "^2.0.0");
		assert.equal(evalsManifest.dependencies["@mariozechner/pi-ai"], "npm:@liuxuedeng/pi-core-ai@1.0.0");
		const generatedManifest = await readManifest(root, "packages/coding-agent/install-lock");
		assert.equal(generatedManifest.dependencies["@liuxuedeng/pi-core"], "^1.0.0");
