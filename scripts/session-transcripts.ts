 ...
 *   --analyze      Spawn agent-core subagents to analyze each transcript file
...
import { parseSessionEntries, type SessionMessageEntry } from "../packages/agent-app/src/session/store.ts";
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("agent-core", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn agent-core: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".agent-core/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn agent-core subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".agent-core/agent/AGENTS.md");
