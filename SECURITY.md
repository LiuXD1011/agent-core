...
This document explains the security model of Pi Core and where its boundaries are.

Pi Core is a coding agent that runs locally within the security boundary of the
...
Pi Core treats the local user account and files writable by that account as
...
access are not security vulnerabilities unless they demonstrate how Pi Core
grants that write access or crosses an operating-system privilege boundary.

Pi Core relies on users installing trustworthy extensions, loading trustworthy
...
If you believe you found a security vulnerability in Pi Core, open a private
