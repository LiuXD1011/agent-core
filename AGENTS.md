...
- Pre-commit blocks lockfile commits unless `PI_CORE_ALLOW_LOCKFILE_CHANGE=1`. Don't bypass unless the user wants the lockfile change committed.
...
- Internal (from issues): `Fixed foo bar ([#123](https://github.com/LiuXD1011/pi-core/issues/123))`
- External contributions: `Added feature X ([#456](https://github.com/LiuXD1011/pi-core/pull/456) by [@username](https://github.com/username))`
...
Pi Core releases are manual for now:

1. Update the per-package `CHANGELOG.md` files.
2. Bump versions (`scripts/sync-versions.mjs` keeps them aligned), regenerate
