...
**Context.** Every asynchronous public harness/lane/Session/Branch/repository/storage method takes an explicit trailing `Context`; synchronous registration (`events.on()`, `hooks.on()`) is contextless, and handlers receive a Context when invoked. Context exists because concurrent calls need independent telemetry parentage and an RPC adapter must carry one request's cancellation as `context.abortSignal`. Shared receivers never retain a caller Context or discover one through `AsyncLocalStorage`. The deleted upstream server/client implemented request-ID cancellation (client signal mapped to `cancel(requestId)`, server deriving a request Context with an `AbortController`); no RPC ingress exists in this repository today. Trace injection/extraction and remote telemetry-parent reconstruction are specified but not implemented (T1, §5.8). Context is process-local invocation authority, never durable data: aborting it does not call `requestAbort()` or write `cancel_requested`.
...
- **C1 — raw RemoteSession (§2.8):** retired. The shipped product deliberately deleted raw `RemoteSession` together with the upstream server/client/worker layers; the process-local Session is the final boundary, and the old transport-lifecycle contract is specification under dispute only for historical reference.
- **R12 — `watchSession` (§5.2):** public method throws `SliceNotImplemented`; the sole stubbed Harness method.
- **T1 — telemetry (§5.8):** span vocabulary declared; production starts only the tool-hook span. Trace propagation is unimplemented; the upstream server/client that carried request-ID cancellation were deleted with the experimental service layers.
...
- **L1 — repository close ownership:** `session/jsonl/repo.ts` still carries the TODO for repository ownership of open handles and all-settled close across the three backends; Memory fails fast.
- **Query bounds:** SQLite `getEntries(ids)` bind-limit chunking and cross-backend query-limit normalization are unverified.
- **Harness DTO cut:** `session/types.ts` still imports `AgentMessage`/`QueueMode`/`ThinkingLevel` from `src/types.ts` instead of declaring harness-local DTOs.
- **SQLite performance debt:** statement caching, per-row usage JSON rewrite, catalog maintenance, and VACUUM policy are measurable debt, not correctness work.
- **Mobile assistant-output handoff:** per-frame durable assistant writes and `message_update` replication amplification remain implemented as whole-message assignment (§3.7, §1.7); a per-frame design must not weaken unknown-outcome recovery.

Known output/tool limitations (verified in source; statements of current behavior, not commitments):

- **Stable configuration-failure error codes are a public compatibility commitment.** Pre-intent unavailability of the captured model fails with code `model_unavailable` (details `{ provider, modelId }`); unavailability of configured active tools fails with `configured_tools_unavailable` (details `{ tools: string[] }`). These exact code strings and detail shapes are the only stable machine-readable surface clients can match on (`runtime/drive/generation.ts`, `deferred.ts`, `structural.ts`).
- **Recovery `turn_end` emission is deliberately inconsistent between paths.** The response path suppresses `turn_end` when recovering or when the next leaf is `tools`/`assistant.retry_wait`, while the tool-placement path emits it at batch completion under different conditions. This asymmetry was recorded as a known, intentional gap, not an accident (`runtime/drive/response.ts`, `runtime/drive/tool-placement.ts`).
- **Replay-safe tools lose their pre-crash output.** Clearing the replay checkpoint deletes the pending tool output before re-executing a replay-safe tool; memoised work emits nothing on replay, so its output is silently lost (`runtime/drive/tools.ts`).
- **Memo and output checkpoint are two separate transactions.** `setMemo` writes only the memo address and the progress write is a separate lane command, so a crash between them loses or duplicates work on replay.
- **Tool errors and cancellations discard streamed partial output.** A thrown tool error becomes an error result built from the error string alone, and a cancelled command keeps partial content only if a durable checkpoint existed.
- **Cancellation classification wins.** A restored cancelled `effect_pending` classifies as cancelled even if its reduced content appears complete; and cancellation reconciliation never replays a restored tool, even one declared `replay:"safe"` — safe replay applies only to ordinary crash recovery (§4.6).
- **Bounded execution-environment output is one view everywhere.** The durable record, model view, and UI view are the same bounded view (a spill is not a second view). The shipped adaptive publisher paces at a harness-global 100 ms floor and 100 KB/s encoded size, collapses to latest, forces terminal publication, and commits the baseline before consumer delivery; stdout and stderr arrive as one merged arrival-ordered view; source-local spill is lazy with 8 MB backpressure and spill failure kills the child. Images and structured tool details are unbounded, shell output is lossy UTF-8, bash's 2 s checkpoint cadence is interim, and the spill sweep is not implemented.
- **Isolated-vm sandbox findings (for any future plugin-sandbox work):** host values cross the membrane only as JSON strings and host callables as integer ids; budgets do not nest across host-re-entered guest callbacks; never create References in a hot path (lazy `FinalizationRegistry` reclamation exhausted a 32 MB isolate under churn); `memoryLimit` disposal is unrecoverable; `getHeapStatisticsSync()` on a stressed isolate hard-aborts the process; and the isolated-vm ABI matrix couples the Node floor to its prebuilds.

Facet integration boundaries (design input from the retired host-facet design; not a commitment, and binding only if session facets are ever integrated):

- Session-facet scoped capabilities reserve host powers to the host: `Session.close`, raw `mutate`/`beginMutation`/`SessionMutator`, the `idGenerator`, backend/storage objects, Branch creation, `setTools`, unscoped hook/event registration, and transport exposure must never be delegated to a facet scope.
- Session facets run beside the real thing and never RPC back into their own process; presentation facets receive nothing raw.
- Facet-owned hook and event registrations are disposed with the facet; the contribution-registry draft/wrap rebuild model (providers and tools are rebuilt from a draft registry, then wrapped) is the reuse model for any scoped registration.

Storage format 4 is still WIP (pre-stabilization): shapes may change in place without migrations; do not invent migration obligations for them. Historical work-package handoffs and the old roadmap were consolidated into this status list; the normative contract remains Parts 0–7 and Part 9.
...
**Session ownership is host-authoritative.** Exactly one worker normally owns a writable Session; create/fork administration may own a destination only until it closes that Session and hands metadata to the worker. Memory, JSONL, and SQLite do not detect a second process opening the same Session for writes; bypassing the host/worker lifecycle is a trusted-host defect. SQLite has no lease, fence, heartbeat, or replacement ownership primitive. A repository still rejects duplicate writable handles and reserves create/open/fork/delete destinations it owns in one process. The host closes a worker before deletion; shared-container deletion removes only that Session's rows in one `BEGIN IMMEDIATE` transaction, while per-file deletion removes its database and WAL/SHM sidecars.
...
**C1 — raw RemoteSession (retired).** The begin/read/commit/end lifecycle was once specified as the RemoteSession transport contract, but no implementation, protocol schema, client facade, server-held scope, worker adapter, or conformance test ever shipped — the shipped product deliberately deleted raw `RemoteSession` together with the upstream server/client/worker layers, in favor of process-local Session plus routed semantic services. The decision is final: the process-local Session is the boundary. If a future design ever commissions a remote Session, it must preserve the same read → decide → commit → process-local publication → end order (invariant 38).
...
Each append checks the same response id is still effect-pending: an append admitted before settlement may commit first; one reaching the line after settlement declines and cannot recreate the list. Frames are auxiliary — absence is legal, they do not prove request completion, and a complete-looking prefix still restores as unknown outcome until settlement commits. Recovery reduces the exact list with `reduceAssistantMessageFrames`, synthesizes the documented partial result, and deletes the list with its next durable decision. Structural summary streams intentionally persist no frames. JSONL records the appends physically until snapshot compaction (J1) even after logical deletion. A future per-frame design would replace this path with tracked pending output in ephemeral scoped storage while preserving unknown-outcome recovery (§0.9).
...
Historical. WP00–WP09 were executed through per-package handoff documents; their normative content was folded into Parts 0–7 and Part 9, and the handoff documents (and the old post-WP05 roadmap) have been removed. The remaining open items are tracked in the §0.9 status list. `harness.md` remains the normative behavior contract; a future work package should state one executable implementation boundary and move newly discovered normative behavior into Parts 0–7 or Part 9 before implementation.
