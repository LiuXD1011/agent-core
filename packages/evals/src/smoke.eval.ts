...
describeEval("Agent Core smoke", { harness: piCodingAgentHarness }, (it) => {
	it("runs a basic prompt end to end", async ({ run }) => {
		...
		expect(result.usage.provider).toBe(process.env.AGENT_CORE_PROVIDER);
		expect(result.usage.model).toBe(process.env.AGENT_CORE_MODEL);
