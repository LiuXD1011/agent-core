...
Agent Core evals are behavioral, model-backed checks for Agent Core workflows. They adapt a real `AgentSession` to `vitest-evals`, run
it in isolated temporary project and agent directories, and attach native Agent Core session artifacts.
...
```bash
AGENT_CORE_PROVIDER=openai AGENT_CORE_MODEL=gpt-5.6-sol npm run eval
```
...
Authentication comes from Agent Core's normal `ModelRuntime`, including subscription credentials and provider API-key
...
native Agent Core session JSONL attachments under `sessions/`. These files may contain prompts, responses, source code, and tool
output.

## Writing evals

Follow [`vitest-evals`](https://github.com/getsentry/vitest-evals) for general suite, judge, assertion, and normalized
trace guidance. Agent Core-specific evals use `createPiCodingAgentHarness(...)` from `src/pi-harness.ts`, with one harness bound
...
describeEval("Agent Core smoke", { harness }, (it) => {
...
### Configuring the Agent Core harness
...
- `noTools`: Agent Core's tool-disable configuration.
...
prompt creates or changes Agent Core resources:

```ts
const result = await run([
	{ type: "prompt", content: "Create an Agent Core extension." },
	{ type: "reload" },
...
Use `output` to expose scenario-specific, JSON-safe behavior without adding that behavior to the generic Agent Core adapter:
...
Harnesses may differ by prompt, tools, skills, model, or any other Agent Core configuration:
...
The Agent Core harness snapshots native session JSONL before deleting its temporary workspace. An eval-only `afterEach` hook
