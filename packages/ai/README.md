...
```bash
npm install @liuxuedeng/agent-core-ai
```

TypeBox exports are re-exported from `@liuxuedeng/agent-core-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@liuxuedeng/agent-core-ai';
import { builtinModels } from '@liuxuedeng/agent-core-ai/providers/all';
...
import { openaiProvider } from '@liuxuedeng/agent-core-ai/providers/openai';
import { openrouterProvider } from '@liuxuedeng/agent-core-ai/providers/openrouter';
import { amazonBedrockProvider } from '@liuxuedeng/agent-core-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@liuxuedeng/agent-core-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@liuxuedeng/agent-core-ai/providers/all';
...
Stored credentials (API keys entered interactively, OAuth tokens) live in a `CredentialStore` — one type-tagged credential per provider. agent-core-ai ships an in-memory default; apps inject persistent storage:

```typescript
import { createModels, type CredentialStore } from '@liuxuedeng/agent-core-ai';
...
API-key credentials use the same discriminator as agent-core's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@liuxuedeng/agent-core-ai';
...
```typescript
import { validateToolCall, type Tool } from '@liuxuedeng/agent-core-ai';
...
```typescript
import { builtinImagesModels } from '@liuxuedeng/agent-core-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@liuxuedeng/agent-core-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@liuxuedeng/agent-core-ai';
import { openAICompletionsApi } from '@liuxuedeng/agent-core-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@liuxuedeng/agent-core-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@liuxuedeng/agent-core-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@liuxuedeng/agent-core-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@liuxuedeng/agent-core-ai/api/<api-id>`.
...
interface OpenAICompletionsCompat {
  ...
  chatTemplateKwargs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort' | 'thinking.budget'; omitWhenOff?: boolean }>; // chat_template_kwargs values; use $var for agent-core-controlled thinking values
  chatTemplateArgs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort' | 'thinking.budget'; omitWhenOff?: boolean }>; // chat_template_args values for thinkingFormat: 'baseten'; use $var for agent-core-controlled thinking values
...
```typescript
import { createModels, type Context } from '@liuxuedeng/agent-core-ai';
import { anthropicProvider } from '@liuxuedeng/agent-core-ai/providers/anthropic';
import { openaiProvider } from '@liuxuedeng/agent-core-ai/providers/openai';
import { googleProvider } from '@liuxuedeng/agent-core-ai/providers/google';
...
- `@liuxuedeng/agent-core-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@liuxuedeng/agent-core-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@liuxuedeng/agent-core-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@liuxuedeng/agent-core-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@liuxuedeng/agent-core-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@liuxuedeng/agent-core-ai/api/bedrock-converse-stream.lazy';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `AGENT_CORE_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @liuxuedeng/agent-core-ai login              # interactive provider selection
npx @liuxuedeng/agent-core-ai login anthropic    # login to specific provider
npx @liuxuedeng/agent-core-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@liuxuedeng/agent-core-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@liuxuedeng/agent-core-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@liuxuedeng/agent-core-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@liuxuedeng/agent-core-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@liuxuedeng/agent-core-ai`.
...
#### 6. Coding Agent Integration (`../agent-app/`)
