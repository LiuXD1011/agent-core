...
```bash
npm install @liuxuedeng/pi-core-ai
```

TypeBox exports are re-exported from `@liuxuedeng/pi-core-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@liuxuedeng/pi-core-ai';
import { builtinModels } from '@liuxuedeng/pi-core-ai/providers/all';
...
import { openaiProvider } from '@liuxuedeng/pi-core-ai/providers/openai';
import { openrouterProvider } from '@liuxuedeng/pi-core-ai/providers/openrouter';
import { amazonBedrockProvider } from '@liuxuedeng/pi-core-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@liuxuedeng/pi-core-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@liuxuedeng/pi-core-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@liuxuedeng/pi-core-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@liuxuedeng/pi-core-ai';
...
```typescript
import { validateToolCall, type Tool } from '@liuxuedeng/pi-core-ai';
...
```typescript
import { builtinImagesModels } from '@liuxuedeng/pi-core-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@liuxuedeng/pi-core-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@liuxuedeng/pi-core-ai';
import { openAICompletionsApi } from '@liuxuedeng/pi-core-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@liuxuedeng/pi-core-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@liuxuedeng/pi-core-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@liuxuedeng/pi-core-ai/api/anthropic-messages';
...
```typescript
import { createModels, type Context } from '@liuxuedeng/pi-core-ai';
import { anthropicProvider } from '@liuxuedeng/pi-core-ai/providers/anthropic';
import { openaiProvider } from '@liuxuedeng/pi-core-ai/providers/openai';
import { googleProvider } from '@liuxuedeng/pi-core-ai/providers/google';
...
- `@liuxuedeng/pi-core-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@liuxuedeng/pi-core-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@liuxuedeng/pi-core-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@liuxuedeng/pi-core-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@liuxuedeng/pi-core-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@liuxuedeng/pi-core-ai/api/bedrock-converse-stream.lazy';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `PI_CORE_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @liuxuedeng/pi-core-ai login              # interactive provider selection
npx @liuxuedeng/pi-core-ai login anthropic    # login to specific provider
npx @liuxuedeng/pi-core-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@liuxuedeng/pi-core-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@liuxuedeng/pi-core-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@liuxuedeng/pi-core-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@liuxuedeng/pi-core-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@liuxuedeng/pi-core-ai`.
