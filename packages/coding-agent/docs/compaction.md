...
For TypeScript definitions in your project, inspect `node_modules/@liuxuedeng/pi-core/dist/`.
...
By default, `reserveTokens` is 16384 tokens (configurable in `~/.pi-core/agent/settings.json` or `<project-dir>/.pi-core/settings.json`). This leaves room for the LLM's response.
...
1. **Find cut point**: Walk backwards from newest message, accumulating token estimates until `keepRecentTokens` (default 20k, configurable in `~/.pi-core/agent/settings.json` or `<project-dir>/.pi-core/settings.json`) is reached
...
Configure compaction in `~/.pi-core/agent/settings.json` or `<project-dir>/.pi-core/settings.json`:
