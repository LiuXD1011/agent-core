...
Discovery rules:
- In `~/.pi-core/agent/skills/` and `.pi-core/skills/`, direct root `.md` files are discovered as individual skills when they have valid skill frontmatter with a non-empty `description`
...
For project-level Claude Code skills, add to `.pi-core/settings.json`:
