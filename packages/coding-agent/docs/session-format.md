...
Sessions can be removed by deleting their `.jsonl` files under `~/.agent-core/agent/sessions/`.

Agent Core also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, agent-core uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([Agent Core](https://github.com/LiuXD1011/agent-core)):
- [`packages/coding-agent/src/core/session-manager.ts`](https://github.com/LiuXD1011/agent-core/blob/main/packages/coding-agent/src/core/session-manager.ts) - Session entry types and SessionManager
- [`packages/coding-agent/src/core/messages.ts`](https://github.com/LiuXD1011/agent-core/blob/main/packages/coding-agent/src/core/messages.ts) - Extended message types (BashExecutionMessage, CustomMessage, etc.)
- [`packages/ai/src/types.ts`](https://github.com/LiuXD1011/agent-core/blob/main/packages/ai/src/types.ts) - Base message types (UserMessage, AssistantMessage, ToolResultMessage)
- [`packages/agent/src/types.ts`](https://github.com/LiuXD1011/agent-core/blob/main/packages/agent/src/types.ts) - AgentMessage union type

For TypeScript definitions in your project, inspect `node_modules/@liuxuedeng/agent-core/dist/` and `node_modules/@liuxuedeng/agent-core-ai/dist/`.
