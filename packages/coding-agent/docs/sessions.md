...
Sessions auto-save to `~/.pi-core/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.

```bash
pi-core -c                  # Continue most recent session
pi-core -r                  # Browse and select from past sessions
pi-core --no-session        # Ephemeral mode; do not save
pi-core --name "my task"    # Set session display name at startup
pi-core --session <path|id> # Use a specific session file or partial session ID
pi-core --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `pi-core -r` opens the same picker at startup.
...
```bash
pi-core --name "Refactor auth module"
pi-core --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `pi-core -r`.
