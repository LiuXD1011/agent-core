...
export function createActionFusionExtension(options: ActionFusionOptions = {}): ExtensionFactory {
	...
	return (pi: ExtensionAPI) => {
		...
		if (!options.tools || options.tools.includes("edit"))
			pi.registerTool<typeof editParameters, EditToolDetails | undefined>({
				...
				async execute(toolCallId, input, signal, onUpdate, ctx) {
					...
					if (then_run && options.canFuse && !options.canFuse()) {
						throw new Error(
							"[then_run:skipped] Use separate mutation and bash calls under the current tool policy.",
						);
					}
		...
		if (!options.tools || options.tools.includes("write"))
