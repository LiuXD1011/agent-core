...
import { type BashToolOptions, createBashToolDefinition, type ExtensionContext } from "../../host.ts";
...
export async function executeMutationThenRun<TDetails>({
	...
	return withFusedFileQueue(absolutePath, async () => {
		...
		if (signal?.aborted || (canRun && !canRun())) {
			throw new Error(
				`${THEN_RUN_SKIPPED} Mutation completed, but cancellation or tool policy prevents the command.`,
			);
		}
