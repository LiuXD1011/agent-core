...
**Custom providers & models:** Add providers via `~/.pi-core/agent/models.json` if they speak a supported API (OpenAI, Anthropic, Google). For custom APIs or OAuth, use extensions. See [docs/models.md](docs/models.md) and [docs/custom-provider.md](docs/custom-provider.md).
...
See `/hotkeys` for the full list. Customize via `~/.pi-core/agent/keybindings.json`. See [docs/keybindings.md](docs/keybindings.md).
...
Sessions auto-save to `~/.pi-core/agent/sessions/` organized by working directory.
...
| Location | Scope |
|----------|-------|
| `~/.pi-core/agent/settings.json` | Global (all projects) |
...
On interactive startup, pi asks before trusting a project folder that contains project-local settings, resources, or project `.agents/skills` and has no saved decision for the folder or a parent folder in `~/.pi-core/agent/trust.json`. Trusting a project allows pi to load `.pi/settings.json` and `.pi` resources, install missing project packages, and execute project extensions.
...
If no extension or saved decision applies, `defaultProjectTrust` controls the fallback behavior. Set it to `"ask"`, `"always"`, or `"never"` in `~/.pi-core/agent/settings.json`, or change it with `/settings`.

`pi config` and package commands use the same project trust flow, except `pi update` never prompts. Pass `--approve` to trust project-local settings for one command or `--no-approve` to ignore them.

Use `/trust` in interactive mode to save a project trust decision for future sessions, including trust for the immediate parent folder. It writes `~/.pi-core/agent/trust.json` only; the current session is not reloaded, so restart pi for changes to take effect.

### Telemetry and update checks

Pi has two separate startup features:

- **Update check:** fetches `https://pi.dev/api/latest-version` to check whether a newer Pi version exists. Disable it with `PI_CORE_SKIP_VERSION_CHECK=1`. Disabling update checks only turns off this check.
- **Install/update telemetry:** after first install or a changelog-detected update, sends an anonymous version ping to `https://pi.dev/api/report-install`. This setting also controls optional provider attribution headers for OpenRouter, Cloudflare, and direct NVIDIA NIM requests. Opt out by setting `enableInstallTelemetry` to `false` in `settings.json`, or by setting `PI_CORE_TELEMETRY=0`. This does not disable update checks; Pi may still contact `pi.dev` for the latest version unless update checks are disabled or offline mode is enabled.

Use `--offline` or `PI_CORE_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.
...
- `~/.pi-core/agent/AGENTS.md` (global)
...
Replace the default system prompt with `.pi/SYSTEM.md` (project) or `~/.pi-core/agent/SYSTEM.md` (global). Append without replacing via `APPEND_SYSTEM.md`.
...
```markdown
<!-- ~/.pi-core/agent/prompts/review.md -->
Review this code for bugs, security issues, and performance problems.
Focus on: {{focus}}
```

Place in `~/.pi-core/agent/prompts/`, `.pi/prompts/`, or a [pi package](#pi-packages) to share with others. See [docs/prompt-templates.md](docs/prompt-templates.md).
...
```markdown
<!-- ~/.pi-core/agent/skills/my-skill/SKILL.md -->
# My Skill
...
Place in `~/.pi-core/agent/skills/`, `~/.agents/skills/`, `.pi/skills/`, or `.agents/skills/` (from `cwd` up through parent directories) or a [pi package](#pi-packages) to share with others. See [docs/skills.md](docs/skills.md).
...
Place in `~/.pi-core/agent/extensions/`, `.pi/extensions/`, or a [pi package](#pi-packages) to share with others. See [docs/extensions.md](docs/extensions.md) and [examples/extensions/](examples/extensions/).

### Themes

Built-in: `dark`, `light`. Themes hot-reload: modify the active theme file and pi immediately applies changes.

Place in `~/.pi-core/agent/themes/`, `.pi/themes/`, or a [pi package](#pi-packages) to share with others. See [docs/themes.md](docs/themes.md).
...
| `PI_CORE_CODING_AGENT` | Set to `true` by the CLI and RPC entry points so child processes can detect that they run inside Pi |
| `PI_CORE_CODING_AGENT_DIR` | Override config directory (default: `~/.pi-core/agent`) |
| `PI_CORE_CODING_AGENT_SESSION_DIR` | Override session storage directory (overridden by `--session-dir`) |
| `PI_CORE_PACKAGE_DIR` | Override package directory (useful for Nix/Guix where store paths tokenize poorly) |
| `PI_CORE_OFFLINE` | Disable startup network operations, including update checks, package update checks, and install/update telemetry |
| `PI_CORE_SKIP_VERSION_CHECK` | Skip the Pi version update check at startup. This prevents the `pi.dev` latest-version request |
| `PI_CORE_TELEMETRY` | Override install/update telemetry and provider attribution headers. Use `1`/`true`/`yes` to enable or `0`/`false`/`no` to disable. This does not disable update checks |
| `PI_CORE_CACHE_RETENTION` | Set to `long` for extended prompt cache (Anthropic: 1h, OpenAI: 24h) |
...
| Variable | Description |
|----------|-------------|
| `PI_CORE_SESSION_ID` | Current session ID |
| `PI_CORE_SESSION_FILE` | Absolute session JSONL path; unset for ephemeral sessions |
| `PI_CORE_PROVIDER` | Currently selected model provider |
| `PI_CORE_MODEL` | Currently selected model ID |
| `PI_CORE_REASONING_LEVEL` | Current effective reasoning level |
