...
By default, Agent Core gives the model four tools: `read`, `write`, `edit`, and `bash`. Add capabilities via skills, prompt templates, extensions, or packages.
...
Agent Core keeps the core small: no built-in MCP, sub-agents, permission popups, plan mode, to-dos, or background bash. Build these workflows as extensions, skills, or packages. See [docs/usage.md](docs/usage.md#design-principles).
