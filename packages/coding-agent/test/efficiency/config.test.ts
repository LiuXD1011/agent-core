...
import { DEFAULT_CONFIG, findConfigPath, loadSolPiConfig } from "../../src/core/efficiency/config.ts";
import {
...
describe("SoL-Pi config", () => {
	...
	it("uses the project config instead of merging the global config", () => {
		const { agentDir, cwd } = fixture();
		writeFileSync(join(agentDir, "sol-pi.json"), JSON.stringify({ version: 1, observationPack: true }));
	...
	it("rejects a non-boolean Online Context Compact value", () => {
		...
		expect(() => loadSolPiConfig(cwd, agentDir, true)).toThrow("SoL-Pi config onlineContextCompact must be boolean");
	});
	...
	it.each([
		...
		expect(() => loadSolPiConfig(cwd, agentDir, true)).toThrow(`SoL-Pi config ${key} must be a non-empty string`);
	});
