...
import {
	...
	fauxText,
	fauxToolCall,
} from "@liuxuedeng/agent-core-ai/providers/faux";
import { describe, expect, it } from "vitest";
import {
	BOUNDARY_COMPACTION_INSTRUCTIONS,
	createOnlineContextCompactExtension,
	POST_COMPACTION_PLAN_REMINDER,
} from "../../src/core/efficiency/extensions/online-context-compact/extension.ts";
import {
	type AgentSession,
	createAgentSession,
	...
	try {
		...
		for (let ordinal = 1; ordinal <= requestedCompactions; ordinal++) {
			...
			responses.push(
				...
				fauxAssistantMessage(
					fauxToolCall("update_plan", { steps: DONE, progress: PROGRESS }, { id: `plan-done-${ordinal}` }),
					{ stopReason: "toolUse" },
				),
			);
		}
		...
		const resourceLoader = new DefaultResourceLoader({
			...
			settingsManager,
			extensionFactories: [
				{ name: `online-context-compact-session-test-${requestedCompactions}`, factory: extension },
			],
			noExtensions: true,
			noSkills: true,
