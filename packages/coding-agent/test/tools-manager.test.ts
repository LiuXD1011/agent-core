...
const originalOffline = process.env.PI_CORE_OFFLINE;
...
afterEach(() => {
	if (originalOffline === undefined) delete process.env.PI_CORE_OFFLINE;
	else process.env.PI_CORE_OFFLINE = originalOffline;
	vi.unstubAllGlobals();
});
