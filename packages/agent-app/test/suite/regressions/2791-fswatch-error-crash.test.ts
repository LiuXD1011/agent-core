...
describe("issue #2791 fs.watch error event crashes process", () => {
	...
	beforeEach(() => {
		...
		const darkThemePath = join(__dirname, "../../../src/ui/terminal/theme/dark.json");
	...
	it("process should survive an error event on the theme FSWatcher", () => {
		const themeModulePath = join(__dirname, "../../../src/ui/terminal/theme/theme.ts").replace(/\\/g, "/");
...
process.env.AGENT_CORE_CODING_AGENT_DIR = "${agentDir}";
...
`,
		...
		try {
			_stdout = execFileSync(process.execPath, [scriptPath], {
				timeout: 10000,
				encoding: "utf-8",
				env: { ...process.env, AGENT_CORE_CODING_AGENT_DIR: agentDir },
