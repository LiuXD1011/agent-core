...
import { estimateTokens } from "../../src/context/summarization/index.ts";
import { createHarness, getUserTexts, type Harness } from "./harness.ts";

type SessionWithCompactionInternals = {
	_checkCompaction: (assistantMessage: AssistantMessage, skipAbortedCheck?: boolean) => Promise<boolean>;
	_runAutoCompaction: (reason: "overflow" | "threshold", willRetry: boolean) => Promise<{ continueRun: boolean }>;
};
...
describe("AgentSession compaction characterization", () => {
	...
	it("notifies extensions when auto-compaction fails", async () => {
		...
		await expect(sessionInternals._runAutoCompaction("threshold", false)).resolves.toMatchObject({
			continueRun: false,
		});
	...
	it("keeps overflow wording when a repeated length stop fills the context window", async () => {
		...
		const runAutoCompactionSpy = vi
			...
			.mockResolvedValue({ continueRun: false });
	...
	it("does not trigger threshold compaction below the threshold or when disabled", async () => {
		...
		const belowThresholdSpy = vi
			...
			.mockResolvedValue({ continueRun: false });
		const disabledSpy = vi.spyOn(disabledInternals, "_runAutoCompaction").mockResolvedValue({ continueRun: false });
