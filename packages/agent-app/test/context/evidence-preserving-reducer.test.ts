...
} from "../../src/context/reduction/index.ts";
...
describe("evidence-preserving reducer", () => {
	...
	it("keeps the Context identifiers that are written to disk", async () => {
		...
		expect(result.content[0]?.text ?? "").toMatch(/^context_evidence_receipt_v1\n/u);
		expect(REDUCER_RECEIPT_SCHEMA).toBe("context-evidence-receipt/1");
		expect(Object.keys(result.details)).toContain("logReduction");
		expect(manager.entries.map((entry) => entry.type === "custom" && entry.customType)).toContain(
			"context-evidence-preserving-reducer-v1",
		);
		expect(manager.customEntryData().every((entry) => entry.schema === "context-evidence-preserving-reducer/1")).toBe(
			true,
		);
	});
	...
	it("uses the Luna reducer model and accepts only verified exact quotes", async () => {
		...
		expect(receipt).toMatch(/authority=Agent retains diagnosis/u);
		...
		expect(notify.mock.calls[0]?.[0]).toMatch(/^Log reduction\nContext · .+ removed from future prompts$/u);
	});
	...
	it.each([false, true])(
		...
		async (failed) => {
			...
			expect(projected).toMatch(/context_evidence_receipt_v1/u);
