...
import { withFusedFileQueue } from "../../src/tools/mutation/file-queue.ts";
import { type ActionFusionOptions, assertUnchangedBeforeCommand } from "../../src/tools/mutation/index.ts";
import { withBuiltInRenderers } from "../../src/ui/renderers/tools/index.ts";
import { componentText, plainTheme } from "./helpers.ts";
import type { BashOperations, ExtensionAPI, ExtensionContext, ToolDefinition } from "./host.ts";
import { createActionFusionExtension } from "./mutation-adapter.ts";
...
describe("action fusion then_run", () => {
	...
	it.each([
		{ label: "default checkout name", cwd: join(tmpdir(), "Context"), path: "target.ts" },
		{ label: "unrelated checkout name", cwd: join(tmpdir(), "plain-checkout"), path: "target.ts" },
		{
			...
			path: "Context/target.ts",
		},
	])("renders file changes and the follow-up command without feature branding ($label)", ({ cwd, path }) => {
		...
		const renderer = withBuiltInRenderers("write", write)!;
		const fused = renderer.renderCall!(fusedArgs, plainTheme, {
			cwd,
			args: fusedArgs,
		} as never);
		const plainArgs = { path, content: "export {};\n" };
		const plain = renderer.renderCall!(plainArgs, plainTheme, {
			cwd,
			args: plainArgs,
		} as never);

		expect(componentText(fused)).toContain("write");
		expect(componentText(fused)).toContain("npm test");
		// A normal path or its OSC 8 hyperlink may contain the repository name.
		expect(componentText(plain)).not.toContain("npm test");
		expect(componentText(plain)).not.toContain("Context · 1 model round-trip avoided");
	});
	...
	it("keeps execution independent of terminal notifications", async () => {
		const dir = await createTempDir();
		const notify = vi.fn();
