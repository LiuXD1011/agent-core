...
import {
	createObservationProjection,
	createObsRecallToolDefinition,
	FULL_SENDS,
	THRESHOLD_BYTES,
} from "../../src/context/observations/index.ts";
...
let cachedSessionRoot: string | undefined;
async function sessionRoot(): Promise<string> {
	const value = await mkdtemp(join(tmpdir(), "observationpack-test-"));
	roots.push(value);
	cachedSessionRoot = value;
	return value;
}

function resultReferencesPi(): FakePi {
	const pi = new FakePi();
	// Test adapter for the removed extension shell: register the native obs_recall
	// tool and drive the projection at the historical "context" hook. The
	// projection instance persists across calls so send counters survive.
	let currentContext: ReturnType<typeof fakeContext> | undefined;
	const deps = {
		getRuntimeRoot: () => runtimeRoot(currentContext ?? fakeContext(cachedSessionRoot ?? "")),
		isRecallAvailable: () => true,
		showSavings: (mechanism: string, saving: string) => {
			if (currentContext) showContextSavings(currentContext, mechanism as ContextTuiMechanism, saving);
		},
	};
	pi.asExtensionApi().registerTool(createObsRecallToolDefinition(deps) as never);
	const projection = createObservationProjection(deps);
	pi.on("context", async (event: { messages: AgentMessage[] }, context: ReturnType<typeof fakeContext>) => {
		currentContext = context;
		return { messages: await projection.project(event.messages) };
	});
	return pi;
}
...
function observationPath(sessionDir: string, id: string): string {
	return join(sessionDir, "context", SESSION_ID, "observation-pack", "objects", `${id}.txt`);
}

function observationObjectsDirectory(sessionDir: string): string {
	return join(sessionDir, "context", SESSION_ID, "observation-pack", "objects");
}
...
describe("observation pack", () => {
	it("registers its public surface without legacy environment flags", () => {
		const pi = resultReferencesPi();
		expect(pi.handlers.has("context")).toBe(true);
		expect(pi.registeredTools.map((tool) => tool.name)).toEqual(["obs_recall"]);
	});

	it("keeps recall independent of terminal rendering", () => {
		const recall = resultReferencesPi().tool("obs_recall");
		expect(recall.renderCall).toBeUndefined();
		expect(recall.renderResult).toBeUndefined();
	});
	...
	it("keeps the first two requests full and reuses one stable placeholder afterwards", async () => {
		...
		const projected = await project(resultReferencesPi(), message, sessionDir, 4);
	...
	it("announces the first measured placeholder saving only in TUI mode", async () => {
		...
		expect(notify.mock.calls[0]?.[0]).toMatch(/^Result reference\nContext · [\d,]+ context tokens avoided$/u);
	});
	...
	it("isolates objects and send counters by Pi session", async () => {
		...
		expect(
			await readFile(join(sessionDir, "context", "session-a", "observation-pack", "objects", `${id}.txt`), "utf8"),
		).toBe(body);
		expect(
			await readFile(join(sessionDir, "context", "session-b", "observation-pack", "objects", `${id}.txt`), "utf8"),
		).toBe(body);
	});
	...
	it("recalls from durable storage after a restart of the extension", async () => {
		...
		const projected = await project(resultReferencesPi(), toolResult(body), sessionDir, 3);
		const id = projected[2]?.match(/id: (obs_[a-f0-9]{24})/u)?.[1];
		expect(id).toBeTruthy();

		const resumed = resultReferencesPi();
	...
	it("fails storage closed when a same-size object holds different content", async () => {
		...
		expect(await project(resultReferencesPi(), message, sessionDir, 3)).toEqual([body, body, body]);
	...
	it("accepts an existing same-content object as idempotent storage", async () => {
		...
		const projected = await project(resultReferencesPi(), message, sessionDir, 3);
	...
	it("fails storage closed when the observation directory is a symlink", async () => {
		...
		await mkdir(join(sessionDir, "context", SESSION_ID, "observation-pack"), { recursive: true });
	...
	it("passes through errors, mixed content, and reducer receipts", async () => {
		...
		const receipt = toolResult(`context_evidence_receipt_v1\n${large}`);
		const compoundReceipt = toolResult("", {
			toolName: "write",
			content: [
				{ type: "text", text: "Successfully wrote 12 bytes to target.ts" },
				{ type: "text", text: `[then_run:succeeded]\ncontext_evidence_receipt_v1\n${large}` },
			],
		});

		expect(await project(resultReferencesPi(), error, sessionDir, 3)).toEqual([large, large, large]);
		expect(await project(resultReferencesPi(), mixed, sessionDir, 3)).toEqual([large, large, large]);
		const receiptText = resultText(receipt);
		expect(await project(resultReferencesPi(), receipt, sessionDir, 3)).toEqual([
			receiptText,
			receiptText,
			receiptText,
		]);
		const compoundText = resultText(compoundReceipt);
		expect(await project(resultReferencesPi(), compoundReceipt, sessionDir, 3)).toEqual([
			compoundText,
			compoundText,
			compoundText,
		]);
