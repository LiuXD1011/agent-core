...
import { ansiLinesToHtml } from "../src/session/export/html/ansi-to-html.ts";
import { createToolHtmlRenderer } from "../src/ui/renderers/html.ts";
...
describe("export HTML tool output whitespace", () => {
	it("preserves whitespace for plain-text tool output lines without preserving template whitespace", () => {
		const css = readFileSync(new URL("../src/session/export/html/template.css", import.meta.url), "utf-8");
