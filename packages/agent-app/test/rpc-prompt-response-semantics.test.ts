...
import type { AgentSessionRuntime } from "../src/app/session-runtime.ts";
import { SettingsManager } from "../src/app/settings-manager.ts";
import { SessionManager } from "../src/session/store.ts";
import { runRpcMode } from "../src/ui/rpc/rpc-mode.ts";
...
vi.mock("../src/app/output-guard.js", () => ({
...
vi.mock("../src/ui/terminal/theme/theme.js", () => ({ theme: {} }));

vi.mock("../src/ui/rpc/jsonl.js", () => ({
			...
			await vi.waitFor(() => {
				...
				expect(responses[0]).toMatchObject({
					...
					error: expect.stringContaining(
						"未找到 fake-provider 的 API 密钥。\n\n使用 /login 通过 OAuth 或 API 密钥登录服务商。参见:",
					),
				});
			});
		} finally {
