...
import type { ToolDefinition } from "../src/extensions/types.ts";
import { type BashOperations, createBashToolDefinition } from "../src/tools/bash.ts";
import { createReadTool, createReadToolDefinition } from "../src/tools/read.ts";
import { createWriteToolDefinition } from "../src/tools/write.ts";
import { withBuiltInRenderers } from "../src/ui/renderers/tools/index.ts";
import { ToolExecutionComponent } from "../src/ui/terminal/components/tool-execution.ts";
import { initTheme, theme } from "../src/ui/terminal/theme/theme.ts";
...
describe("ToolExecutionComponent parity", () => {
	...
	test("bash renderer does not duplicate final full output truncation details", async () => {
		...
		const component = new ToolExecutionComponent(
			...
			{ command: "generate output" },
			{},
			withBuiltInRenderers("bash", tool as ToolDefinition),
			createFakeTui(),
			process.cwd(),
		);
	...
	test("does not duplicate built-in headers when passed the active built-in definition", () => {
		const component = new ToolExecutionComponent(
			...
			{ path: "README.md" },
			{},
			withBuiltInRenderers("read", createReadToolDefinition(process.cwd()) as ToolDefinition),
			createFakeTui(),
			process.cwd(),
		);
	...
	test("trims trailing blank display lines from write previews", () => {
		const component = new ToolExecutionComponent(
			...
			{},
			withBuiltInRenderers("write", createWriteToolDefinition(process.cwd()) as ToolDefinition),
			createFakeTui(),
			process.cwd(),
		);
		const rendered = stripAnsi(component.render(120).join("\n"));
