...
import { createAgentSessionFromServices, createAgentSessionServices } from "../src/app/services.ts";
import { SettingsManager } from "../src/app/settings-manager.ts";
import { type CreateAgentSessionOptions, createAgentSession, type InlineExtension } from "../src/sdk.ts";
	...
	it("uses the configured list as the initial built-in selection", async () => {
		...
		expect(
			session
				...
				.map((tool) => tool.name)
				.sort(),
		).toEqual(["bash", "edit", "find", "grep", "ls", "powershell", "read", "update_plan", "write"]);
