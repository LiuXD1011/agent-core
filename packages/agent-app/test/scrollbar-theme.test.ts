...
import { validateThemeJson } from "../src/app/theme-json.ts";
import { loadThemeFromPath, setThemeJsonValidator } from "../src/ui/terminal/theme/theme.ts";
...
function loadDarkTheme(): { name: string; colors: Record<string, string | number> } {
	return JSON.parse(readFileSync(new URL("../src/ui/terminal/theme/dark.json", import.meta.url), "utf8")) as {
		name: string;
