...
vi.mock("fs", async (importOriginal) => {
	...
	return {
		...
		readFileSync: (path: Fs.PathOrFileDescriptor, options?: unknown) =>
			path === "/proc/version" ? "Linux isolated clipboard test" : original.readFileSync(path, options as never),
	};
});
	...
	test("WSL: passes PowerShell path directly instead of through a custom env var", async () => {
		...
		mocks.spawnSync.mockImplementation((command, args, options) => {
			...
			if (command === "powershell.exe") {
				...
				expect(spawnOptions.env?.AGENT_CORE_WSL_CLIPBOARD_IMAGE_PATH).toBeUndefined();
