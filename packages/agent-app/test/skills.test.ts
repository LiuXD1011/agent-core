...
import type { ResourceDiagnostic } from "../src/app/diagnostics.ts";
import { formatSkillsForPrompt, loadSkills, loadSkillsFromDir, type Skill } from "../src/app/skills.ts";
...
describe("skills", () => {
	...
	describe("loadSkills with options", () => {
		...
		it("should expand ~ in skillPaths", () => {
			const homeSkillsDir = join(homedir(), ".agent-core/agent/skills");
			const { skills: withTilde } = loadSkills({
				agentDir: emptyAgentDir,
				cwd: emptyCwd,
				skillPaths: ["~/.agent-core/agent/skills"],
				includeDefaults: true,
			});
