...
describe("createAgentSession provider attribution headers", () => {
	...
	it("does not inject attribution headers for OpenRouter models", async () => {
	...
	it("does not inject attribution headers with explicit empty options", async () => {
		const headers = await captureHeaders(createModel("openrouter", "https://openrouter.ai/api/v1"), {});
	...
	it("does not inject attribution headers for custom OpenRouter providers", async () => {
	...
	it("does not infer attribution from a malformed base URL", async () => {
	...
	it("preserves explicit provider and request headers", async () => {
		const headers = await captureHeaders(createModel("openrouter", "https://openrouter.ai/api/v1"), {
			providerHeaders: {
	...
	it("does not add NVIDIA NIM attribution headers with explicit empty options", async () => {
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("agent-core");
	});
