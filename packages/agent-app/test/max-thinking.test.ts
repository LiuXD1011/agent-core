...
import { loadThemeFromPath } from "../src/ui/terminal/theme/theme.ts";
...
describe("max thinking level", () => {
	...
	it("falls back to thinkingXhigh for legacy themes", () => {
		...
		const darkTheme = JSON.parse(readFileSync(join(currentDir, "../src/ui/terminal/theme/dark.json"), "utf8")) as {
			name: string;
