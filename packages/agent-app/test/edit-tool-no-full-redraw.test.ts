...
import { Container, type Terminal, Text, type TUI, TuiMainScreen } from "@liuxuedeng/agent-core-tui";
import { afterEach, beforeAll, describe, expect, it } from "vitest";
import type { ToolDefinition } from "../src/extensions/types.ts";
import { createEditToolDefinition } from "../src/tools/edit.ts";
import { computeEditsDiff, type Edit } from "../src/tools/edit-diff.ts";
...
`,
		...
		const component = new ToolExecutionComponent(
			...
			{ path: filePath, edits },
			{},
			withBuiltInRenderers("edit", createEditToolDefinition(process.cwd()) as ToolDefinition),
			tui,
			process.cwd(),
		);
