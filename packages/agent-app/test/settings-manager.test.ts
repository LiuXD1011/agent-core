...
import { DEFAULT_HTTP_IDLE_TIMEOUT_MS } from "../src/app/http-dispatcher.ts";
...
describe("SettingsManager", () => {
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".agent-core", "settings.json");
	...
	describe("project trust", () => {
		it("should skip project settings when project is not trusted", () => {
			...
			writeFileSync(join(projectDir, ".agent-core", "settings.json"), JSON.stringify({ theme: "project" }));
	...
	describe("project settings directory creation", () => {
		it("should not create .pi folder when only reading project settings", () => {
			...
			rmSync(join(projectDir, ".agent-core"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .pi folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".agent-core"))).toBe(false);
		...
		it("should create .pi folder when writing project settings", async () => {
			...
			expect(existsSync(join(projectDir, ".agent-core"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".agent-core", "settings.json"))).toBe(true);
		});
	});
	...
	describe("httpIdleTimeoutMs", () => {
		...
		it("should use merged global and project settings", () => {
			...
			writeFileSync(join(projectDir, ".agent-core", "settings.json"), JSON.stringify({ httpIdleTimeoutMs: 0 }));
	...
	describe("defaultTools", () => {
		it("loads global defaults and lets project settings replace them", () => {
			...
			writeFileSync(join(projectDir, ".agent-core", "settings.json"), JSON.stringify({ defaultTools: ["grep"] }));
	...
	describe("getSessionDir", () => {
		...
		it("should return project sessionDir, overriding global", () => {
			...
			writeFileSync(join(projectDir, ".agent-core", "settings.json"), JSON.stringify({ sessionDir: "./sessions" }));
			const manager = SettingsManager.create(projectDir, agentDir);
