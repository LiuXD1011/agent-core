...
import { createEventBus, type EventBus } from "../app/event-bus.ts";
import type { ExecOptions } from "../app/exec.ts";
import { execCommand } from "../app/exec.ts";
import { readPiManifest } from "../app/pi-manifest.ts";
import { createSyntheticSourceInfo } from "../app/source-info.ts";
...
/** CLI/UI entries provide bundled presentation modules, never the business layer. */
export function registerExtensionModules(modules: Record<string, unknown>): void {
	Object.assign(VIRTUAL_MODULES, modules);
}
...
function getAliases(): Record<string, string> {
	...
	const packageIndex = path.resolve(__dirname, isTypeScriptSourceRuntime ? "../index.ts" : "../index.js");
		...
		...(isBunBinary || isNodeSeaBinary || isBundledNode
			...
			: isTypeScriptSourceRuntime
				? {
						...
						tsconfigPaths: true,
						alias: {
							"@liuxuedeng/agent-core": path.resolve(
