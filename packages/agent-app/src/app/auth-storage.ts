...
function getAuthRevision(path: string): string | undefined {
	try {
...
export class AuthStorage implements CredentialStore {
	...
	private constructor(storage: AuthStorageBackend, authPath?: string) {
		...
		if (authPath) {
			const revision = getAuthRevision(authPath);
	...
	reload(): void {
		...
		try {
			this.storage.withLock((current) => {
				content = current;
				revision = this.authPath ? getAuthRevision(this.authPath) : undefined;
	...
	private async reloadFromStorageAsync(options?: AuthOperationOptions): Promise<AuthStorageData> {
		return this.storage.withLockAsync(async (content) => {
			...
			const revision = this.authPath ? getAuthRevision(this.authPath) : undefined;
	...
	private async readLatestData(options?: AuthOperationOptions): Promise<AuthStorageData> {
		...
		const revision = getAuthRevision(this.authPath);
