...
import { restoreContextArchives } from "../session/artifacts.ts";
import { assertSessionCwdExists } from "../session/cwd.ts";
import { SessionManager } from "../session/store.ts";
import { resolvePath } from "../utils/paths.ts";
import type { AgentSession } from "./application.ts";
...
export class AgentSessionRuntime {
	...
	async importFromJsonl(inputPath: string, cwdOverride?: string): Promise<{ cancelled: boolean }> {
		...
		assertSessionCwdExists(sessionManager, this.cwd);
		// Materialize any context archives the export carried so recall keeps
		// working on this machine; the payloads never enter the LLM context.
