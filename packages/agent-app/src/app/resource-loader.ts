...
import { assertThemeNameIsValid, resolveThemeColors, type ThemeResource } from "./theme-data.ts";
import { validateThemeJson } from "./theme-json.ts";

export type { ResourceCollision, ResourceDiagnostic } from "./diagnostics.ts";

import { type ContextConfig, loadContextConfig } from "../context/config.ts";
import {
	clearExtensionCache,
...
export interface ResourceLoader {
	...
	/** Context feature config resolved for this loader's cwd/agentDir/trust. Optional for custom loaders. */
...
export class DefaultResourceLoader implements ResourceLoader {
	...
	private _contextConfig: ContextConfig | undefined;
	...
	/** Context feature config resolved during the last reload for this loader's cwd/agentDir/trust. */
	get contextConfig(): ContextConfig {
		return this._contextConfig ?? loadContextConfig(this.cwd, this.agentDir, this.settingsManager.isProjectTrusted());
	}
	...
	async reload(options?: ResourceLoaderReloadOptions): Promise<void> {
		...
		// Context mechanisms are native to the session runtime (AgentSession);
		// they are resolved here once so the session can read the effective config.
		this._contextConfig = loadContextConfig(this.cwd, this.agentDir, this.settingsManager.isProjectTrusted());
		...
		for (const p of this.additionalThemePaths) {
			...
			if (!existsSync(resolved) && !this.themeDiagnostics.some((d) => d.path === resolved)) {
				this.themeDiagnostics.push({ type: "error", message: "ThemeResource path does not exist", path: resolved });
			}
		}
	...
	private loadThemesFromDir(dir: string, themes: ThemeResource[], diagnostics: ResourceDiagnostic[]): void {
		if (!existsSync(dir)) {
			return;
		}
	...
	private loadThemeFromFile(filePath: string, themes: ThemeResource[], diagnostics: ResourceDiagnostic[]): void {
		try {
			const json = validateThemeJson(filePath, JSON.parse(stripBom(readFileSync(filePath, "utf8"))));
			assertThemeNameIsValid(json.name);
			resolveThemeColors(json.colors, json.vars);
			themes.push({ name: json.name, sourcePath: filePath, json });
		} catch (error) {
	...
	private dedupeThemes(themes: ThemeResource[]): { themes: ThemeResource[]; diagnostics: ResourceDiagnostic[] } {
