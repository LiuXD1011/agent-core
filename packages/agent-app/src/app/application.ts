...
import { createObservationProjection, createObsRecallToolDefinition } from "../context/observations/index.ts";
import { OnlineCompactionCoordinator } from "../context/online-compact.ts";
import { createReducerRunner } from "../context/reduction/index.ts";
import {
...
import type { ToolHtmlRenderer } from "../session/export/html/index.ts";
import { exportSessionHtmlFromManager, exportSessionJsonlFromManager } from "../session/export/jsonl.ts";
import type { BashExecutionMessage, CustomMessage } from "../session/messages.ts";
import { snapshotRecord } from "../session/snapshot.ts";
import type { BranchSummaryEntry, CompactionEntry, SessionEntry, SessionManager } from "../session/store.ts";
import { getLatestCompactionEntry } from "../session/store.ts";
import { type BashOperations, createLocalBashOperations } from "../tools/bash.ts";
import { createAllToolDefinitions } from "../tools/index.ts";
import { createFusedMutationToolDefinitions } from "../tools/mutation/index.ts";
import { createUpdatePlanToolDefinition } from "../tools/plan.ts";
...
export class AgentSession {
	...
	private _onlineCompact: OnlineCompactionCoordinator | undefined;
	private _observationProjection: ReturnType<typeof createObservationProjection> | undefined;
	private _nativeReducer: ReturnType<typeof createReducerRunner> | undefined;
	private _fusedMutations: ReturnType<typeof createFusedMutationToolDefinitions> | undefined;
	private _obsRecallDefinition: ReturnType<typeof createObsRecallToolDefinition> | undefined;
	private _contextRuntimeRoot: (() => string) | undefined;
	private _nativeContextHooksInstalled = false;
	private _baseAfterToolCall: NonNullable<Agent["afterToolCall"]> | undefined;

	/**
	 * Context and tool policies join the base registry and hooks compose
	 * with the agent's request/turn pipeline directly — no extension loading, so
	 * `--no-extensions` never disables them. Each mechanism yields to an
	 * extension that already owns its tool name.
	 */
	private _installNativeContext(extensionsResult: {
		extensions: ReadonlyArray<{ tools: ReadonlyMap<string, unknown> }>;
	}): void {
		const contextConfig = this._resourceLoader.contextConfig ?? loadContextConfig(this._cwd);
		const ownedByExtension = (name: string): boolean =>
			extensionsResult.extensions.some((extension) => extension.tools.has(name));

		this._onlineCompact = undefined;
		this._observationProjection = undefined;
		this._nativeReducer = undefined;
		this._fusedMutations = undefined;
		this._obsRecallDefinition = undefined;
		this._contextRuntimeRoot = undefined;

		const hostOwnsMutationTool = ["edit", "write", "bash"].some(
			(name) =>
				this._customTools.some((tool) => tool.name === name) ||
				(this._baseToolsOverride && name in this._baseToolsOverride),
		);
		if (
			contextConfig.mutationCommands &&
			!hostOwnsMutationTool &&
			!ownedByExtension("edit") &&
			!ownedByExtension("write")
		) {
			this._fusedMutations = createFusedMutationToolDefinitions({
				canFuse: () =>
					this.agent.state.tools.some((tool) => tool.name === "bash") &&
					!this._extensionRunner.hasHandlers("tool_call") &&
					!this._extensionRunner.hasHandlers("tool_result"),
				bashOptions: {
		...
		if ((contextConfig.resultReferences || contextConfig.logReduction) && this.sessionManager.getSessionDir()) {
			const runtimeRoot = (): string => {
				...
				return `${sessionDir}/context/${sessionId}`.replaceAll("//", "/");
			};
			this._contextRuntimeRoot = runtimeRoot;
			this._obsRecallDefinition = createObsRecallToolDefinition({
				getRuntimeRoot: runtimeRoot,
				isRecallAvailable: () => this.agent.state.tools.some((tool) => tool.name === "obs_recall"),
				showSavings: (mechanism, saving) => this._showContextSavings(mechanism, saving),
			});
			this._observationProjection = createObservationProjection({
				getRuntimeRoot: runtimeRoot,
				isRecallAvailable: () => this.agent.state.tools.some((tool) => tool.name === "obs_recall"),
				showSavings: (mechanism, saving) => this._showContextSavings(mechanism, saving),
			});
		...
		if (contextConfig.logReduction) {
			this._nativeReducer = createReducerRunner({
				reducerProvider: contextConfig.logReductionProvider,
				reducerModel: contextConfig.logReductionModel,
			});
		}

		if (contextConfig.boundaryCompaction && !ownedByExtension("update_plan")) {
			this._onlineCompact = new OnlineCompactionCoordinator(
				{
					...
					getModelContextWindow: () => this.model?.contextWindow ?? null,
					compact: async (customInstructions) => {
						const outcome = await this._runAutoCompaction("threshold", false, customInstructions);
						return outcome.result ? { summary: outcome.result.summary } : null;
					},
				...
				},
				{
					...
					cacheWriteReadRatio: contextConfig.cacheWriteReadRatio,
				},
			);
			this._onlineCompact.restore();
		}

		this._installNativeContextHooks();
	}

	/** Route an context savings notice to the TUI when one is attached. */
	private _showContextSavings(mechanism: string, saving: string): void {
		const runner = this._extensionRunner;
		if (!runner?.hasUI?.()) return;
		const ui = runner.getUIContext();
		const message = `${mechanism}: ${saving}`;
		try {
			ui.notify?.(message, "info");
			ui.setStatus?.(mechanism, `${mechanism} · ${saving}`);
			setTimeout(() => ui.setStatus?.(mechanism, undefined), 4_000).unref?.();
		} catch {
			// Savings display is best-effort; never break the mechanism for UI failures.
		}
	}

	/** Compose native hooks with the agent pipeline exactly once. */
	private _installNativeContextHooks(): void {
		if (this._nativeContextHooksInstalled) return;
		this._nativeContextHooksInstalled = true;

		const previousTransformContext = this.agent.transformContext;
		this.agent.transformContext = async (messages, signal) => {
			const transformed = previousTransformContext ? await previousTransformContext(messages, signal) : messages;
			let projected = transformed;
			if (this._observationProjection && this.agent.state.tools.some((tool) => tool.name === "obs_recall")) {
				projected = await this._observationProjection.project(transformed);
			}
			this._onlineCompact?.observeMessages(projected);
			this._onlineCompact?.recordProviderRequest();
			return projected;
		};

		this._baseAfterToolCall = this.agent.afterToolCall ?? undefined;
		this.agent.afterToolCall = async (input, signal) => {
			const previous = await this._baseAfterToolCall?.(input, signal);
			if (!this._nativeReducer) return previous;
			const event = {
				type: "tool_result" as const,
				toolName: input.toolCall.name,
				toolCallId: input.toolCall.id,
				input: input.args as Record<string, unknown>,
				content: previous?.content ?? input.result.content,
				details: previous?.details ?? input.result.details,
				isError: previous?.isError ?? input.isError,
				usage: previous?.usage ?? input.result.usage,
			};
			const reduced = (await this._nativeReducer(event, {
				getRuntimeRoot: () => this._contextRuntimeRoot?.() ?? "",
				...
				getSignal: () => signal,
				showSavings: (mechanism, saving) => this._showContextSavings(mechanism, saving),
			})) as { content: typeof input.result.content; details: unknown; isError: boolean } | undefined;
			if (!reduced) return previous;
			return {
				content: reduced.content ?? previous?.content ?? input.result.content,
	...
	private _installAgentNextTurnRefresh(): void {
		...
		this.agent.prepareNextTurnWithContext = async (turn, signal) => {
			let context = await this._compactBeforeNextAssistantResponse(turn.context);
			if (this._onlineCompact) {
				this._onlineCompact.observeMessages(context.messages);
				if (await this._onlineCompact.maybeCompactBeforeNextTurn(signal)) {
					context = { ...context, messages: this.agent.state.messages.slice() };
				}
			}
	...
	private _emit(event: AgentSessionEvent): void {
		for (const l of this._eventListeners) {
			try {
				void Promise.resolve(l(snapshotRecord(event))).catch((error: unknown) => {
					console.error(`[agent-session] event listener failed for ${event.type}:`, error);
				});
			} catch (error) {
				// A broken observer must never break the run or block record
				// persistence; the record is already committed by the time
				// listeners run (see _handleAgentEvent).
				console.error(`[agent-session] event listener failed for ${event.type}:`, error);
			}
		}
	}
	...
	private _handleAgentEvent = async (event: AgentEvent): Promise<void> => {
		...
		// Emit to extensions first: their message_end replacement is part of the
		// documented record contract and becomes the committed content.
		await this._emitExtensionEvent(event);

		// Commit the authoritative record BEFORE notifying observers: a slow or
		// throwing UI must never block persistence or rewrite history (design §7.1).
		if (event.type === "message_end") {
			...
			// Observers get a snapshot: they cannot mutate the committed record.
			this._emit({ ...event, message: event.message });
		} else {
	...
	private async _emitExtensionEvent(event: AgentEvent): Promise<void> {
		if (event.type === "agent_start") {
			...
			await this._extensionRunner.emit({ type: "agent_end", messages: snapshotRecord(event.messages) });
			...
			await this._extensionRunner.emit(snapshotRecord(extensionEvent));
			...
			const replacement = await this._extensionRunner.emitMessageEnd(snapshotRecord(extensionEvent));
			if (replacement) {
	...
	async steer(text: string, images?: ImageContent[]): Promise<void> {
		...
		this._onlineCompact?.recordCorrection();
	...
	private async _checkCompaction(assistantMessage: AssistantMessage, skipAbortedCheck = true): Promise<boolean> {
		...
		if (contextOverflow || recoverableLength) {
			...
			if (!willRetry) {
				return (await this._runAutoCompaction("overflow", false)).continueRun;
			}
			...
			return (await this._runAutoCompaction("overflow", willRetry)).continueRun;
		}
		...
		if (shouldCompact(contextTokens, contextWindow, settings)) {
			return (await this._runAutoCompaction("threshold", false)).continueRun;
		}
		return false;
	}
	 ...
	 * @returns The committed result and a separate post-run continuation decision
	 */
	private async _runAutoCompaction(
		...
		try {
			...
			if (willRetry) {
				...
				return { result, continueRun: true };
			}

			// Auto-compaction can complete while follow-up/steering/custom messages are waiting.
			// Continue once so queued messages are delivered.
			return { result, continueRun: this.agent.hasQueuedMessages() };
		} catch (error) {
	...
	private _buildRuntime(options: {
		...
		this._installNativeContext(extensionsResult);
		if (this._onlineCompact) {
			const coordinator = this._onlineCompact;
			baseToolDefinitions.update_plan = createUpdatePlanToolDefinition({
				updatePlan: (input) => coordinator.handlePlanUpdate(input),
			});
		}
		if (this._obsRecallDefinition) {
			baseToolDefinitions.obs_recall = this._obsRecallDefinition;
		}
		if (this._fusedMutations) {
			baseToolDefinitions.edit = this._fusedMutations.edit as ToolDefinition;
			baseToolDefinitions.write = this._fusedMutations.write as ToolDefinition;
		}
		...
		if (!this._baseToolsOverride && this._obsRecallDefinition) defaultActiveToolNames.push("obs_recall");
	...
	async exportToHtml(
		outputPath?: string,
		options: { themeName?: string; toolRenderer?: ToolHtmlRenderer } = {},
	): Promise<string> {
		return await exportSessionHtmlFromManager(this.sessionManager, {
			outputPath,
			themeName: options.themeName ?? this.settingsManager.getTheme(),
			state: this.state,
			toolRenderer: options.toolRenderer,
		});
	}
	...
	exportToJsonl(outputPath?: string): string {
		return exportSessionJsonlFromManager(this.sessionManager, outputPath);
	}
