...
import { InMemoryCodingAgentModelsStore } from "../app/models-store.ts";
...
export async function checkProviderAuth(
	...
	if (cliModel) {
		...
		if (resolved.error || !resolved.model) {
			throw new AuthCommandError(resolved.error ?? `无法解析模型 "${cliModel}"`);
		}
		provider = resolved.model.provider;
	}
	if (!provider) throw new AuthCommandError("无法解析认证服务商");
