...
import { getExperimentalToolSampling } from "../app/experimental.ts";
...
/** Minimum interval between streaming tool updates, shared with the shell renderer's clock. */
...
export const bashToolSystemPromptContribution = {
	...
	guidelines: ["You can inspect AGENT_CORE_* environment variables for current model and session details."],
} as const;
...
function resolveSpawnContext(
	...
	delete env.AGENT_CORE_SESSION_ID;
	delete env.AGENT_CORE_SESSION_FILE;
	delete env.AGENT_CORE_PROVIDER;
	delete env.AGENT_CORE_MODEL;
	delete env.AGENT_CORE_REASONING_LEVEL;
	if (exposeSessionEnvironment && ctx) {
		const model = ctx.model;
		env.AGENT_CORE_SESSION_ID = ctx.sessionManager.getSessionId();
		const sessionFile = ctx.sessionManager.getSessionFile();
		if (sessionFile) env.AGENT_CORE_SESSION_FILE = sessionFile;
		if (model) {
			env.AGENT_CORE_PROVIDER = model.provider;
			env.AGENT_CORE_MODEL = model.id;
		}
		if (ctx.thinkingLevel) env.AGENT_CORE_REASONING_LEVEL = ctx.thinkingLevel;
	}
...
export interface BashToolOptions {
	...
	shellPath?: string;
	/** Expose current Pi session metadata as AGENT_CORE_* environment variables. Default: true */
	exposeSessionEnvironment?: boolean;
