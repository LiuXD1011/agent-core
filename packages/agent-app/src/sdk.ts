...
import { Agent, type AgentMessage, setDefaultStreamFn, type ThinkingLevel } from "@liuxuedeng/agent-core-agent";
import { clampThinkingLevel, type Message, type Model, streamSimple } from "@liuxuedeng/agent-core-ai/compat";
import { AgentSession } from "./app/application.ts";
import { formatNoModelsAvailableMessage } from "./app/auth-guidance.ts";
import { DEFAULT_THINKING_LEVEL } from "./app/defaults.ts";
import { findInitialModel } from "./app/model-resolver.ts";
import { ModelRuntime } from "./app/model-runtime.ts";
import { mergeProviderAttributionHeaders } from "./app/provider-attribution.ts";
import type { ResourceLoader } from "./app/resource-loader.ts";
import { DefaultResourceLoader } from "./app/resource-loader.ts";
import { SettingsManager } from "./app/settings-manager.ts";
import { time } from "./app/timings.ts";
import { getAgentDir } from "./config.ts";
import type { ExtensionRunner, LoadExtensionsResult, SessionStartEvent, ToolDefinition } from "./extensions/index.ts";
import { projectArchiveReferences, restoreContextArchives } from "./session/artifacts.ts";
import { convertToLlm } from "./session/messages.ts";
import { getDefaultSessionDir, SessionManager } from "./session/store.ts";
import {
	createBashTool,
	createCodingTools,
	createEditTool,
...
export interface CreateAgentSessionOptions {
	...
	cwd?: string;
	/** Global config directory. Default: ~/.agent-core/agent */
	agentDir?: string;
...
export type { PromptTemplate } from "./app/prompt-templates.ts";
export * from "./app/session-runtime.ts";
export type { Skill } from "./app/skills.ts";
export type {
	ExtensionAPI,
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	restoreContextArchives(sessionManager);
	...
	const defaultActiveToolNames = ["read", "bash", "edit", "write", "update_plan", "obs_recall"];
	...
	const convertToLlmWithBlockImages = (messages: AgentMessage[]): Message[] => {
		const converted = convertToLlm(projectArchiveReferences(messages, sessionManager));
