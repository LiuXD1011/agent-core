...
export function formatPlanSnapshot(steps: readonly PlanStep[]): string {
	return `<context-plan task_status="active">${JSON.stringify({ steps })}</context-plan>`;
}
...
export function resolveKeepRecentTokens(value: number | undefined): number {
	...
	if (!Number.isSafeInteger(resolved) || resolved < 1) {
		throw new Error("Context compaction keepRecentTokens must be a positive safe integer");
	}
	return resolved;
}
