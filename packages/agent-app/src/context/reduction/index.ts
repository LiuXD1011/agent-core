 ...
 */
/**
 * Verified log reduction - delegate the first read of a long build or test
 ...
 * provider/model selected by the top-level Context config, and accepts the
 ...
 *
 * The top-level Context config enables this mechanism. Provider selection and
 * authentication remain with Pi; storage and run identity come from the session.
 */

import type { ExtensionContext, ToolResultEvent } from "../../extensions/types.ts";
import { formatSavingsBytes } from "../../utils/context-format.ts";
...
export async function reduceToolResult(
	...
	notifySavings?: (mechanism: string, saving: string) => void,
	...
	if (notifySavings) {
		notifySavings("Log reduction", formatSavingsBytes(archive.bytes - receiptBytes));
	}
	return {
...
/** Runtime surfaces the native session assembly provides per reduction call. */
...
function structuralContext(deps: ReducerRuntimeDeps, signal: AbortSignal | undefined): ExtensionContext {
	return {
		modelRegistry: deps.modelRegistry,
		model: deps.getModel(),
		signal,
		sessionManager: { getSessionDir: () => deps.getRuntimeRoot(), getSessionId: () => "native" },
	} as unknown as ExtensionContext;
}

/**
 * Native reducer runner: returns undefined when the result is not reducible or
 * the reduction fell back, and the reduced replacement when a receipt verified.
 */
export function createReducerRunner(options: EvidencePreservingReducerOptions = {}) {
	const states = new Map<string, { config: ReducerConfig; journal: Journal }>();
	return async (event: ToolResultEvent, deps: ReducerRuntimeDeps): Promise<ReducedToolResult | undefined> => {
		let root: string;
		try {
			root = deps.getRuntimeRoot();
		} catch {
			return undefined;
		}
		...
		if (!state) {
			...
			state = { config, journal: createJournal({ appendEntry: deps.appendSessionEntry }, config) };
			states.set(root, state);
		}
		const context = structuralContext(deps, deps.getSignal());
		return await reduceToolResult(state.journal, state.config, event, context, deps.showSavings);
	};
}
