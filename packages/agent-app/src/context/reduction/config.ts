...
export const REDUCER_EVENT_TYPE = "context-evidence-preserving-reducer-v1" as const;
export const REDUCER_EVENT_SCHEMA = "context-evidence-preserving-reducer/1" as const;
export const REDUCER_RECEIPT_SCHEMA = "context-evidence-receipt/1" as const;
export const REDUCER_RECEIPT_PREFIX = "context_evidence_receipt_v1" as const;
...
export const DEFAULT_REDUCER_PROVIDER = "$current";
export const DEFAULT_REDUCER_MODEL = "$current";

export const DIAGNOSTIC_COMMAND =
	/(?:^|[;&|()\s])(?:lake\s+build|lake\s+env\s+lean|lean|coq|cargo(?:\s+(?:build|test|check))?|zig\s+build|pytest|python(?:3)?\s+-m\s+(?:pytest|unittest|py_compile)|ctest|cmake\s+--build|ninja|make|npm\s+(?:test|run\s+(?:check|test|lint|typecheck|build))|pnpm\s+(?:test|check|lint|typecheck|build)|yarn\s+(?:test|check|lint|typecheck|build)|node\s+[^;&|\n]*vitest[^;&|\n]*--run|go\s+test|bazel\s+test)(?:\s|$)/i;
