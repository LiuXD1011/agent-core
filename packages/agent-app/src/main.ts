...
import type { ImageContent } from "@liuxuedeng/agent-core-ai";
...
import { AuthStorage, ReadOnlyAuthStorage } from "./app/auth-storage.ts";
import { applyHttpProxySettings, configureHttpDispatcher } from "./app/http-dispatcher.ts";
import { resolveCliModel } from "./app/model-resolver.ts";
import { ModelRuntime } from "./app/model-runtime.ts";
import { restoreStdout, takeOverStdout } from "./app/output-guard.ts";
...
import { type CreateAgentSessionRuntimeFactory, createAgentSessionRuntime } from "./app/session-runtime.ts";
import { collectSettingsDiagnostics, deduplicateDiagnostics } from "./app/settings-diagnostics.ts";
import { SettingsManager } from "./app/settings-manager.ts";
import { validateThemeJson } from "./app/theme-json.ts";
import { printTimings, resetTimings, time } from "./app/timings.ts";
...
import { exportSessionHtmlFromFile } from "./session/export/jsonl.ts";
import { assertValidSessionId, SessionManager } from "./session/store.ts";
import { InteractiveMode, runPrintMode, runRpcMode } from "./ui/index.ts";
import { initTheme, setThemeJsonValidator, stopThemeWatcher } from "./ui/terminal/theme/theme.ts";
import { isLocalPath, normalizePath, resolvePath } from "./utils/paths.ts";
import { cleanupWindowsSelfUpdateQuarantine } from "./utils/windows-self-update.ts";

const EXTENSION_LOAD_FAILURE_HINT = `提示：使用 "${APP_NAME} -ne" 在不加载扩展的情况下启动。`;
...
async function runAuthCommand(args: string[]): Promise<boolean> {
	...
	try {
		...
		const message = error instanceof AuthCommandError ? error.message : "解析认证命令失败";
	...
	if (parsed.unknownFlags.size > 0) {
		...
		console.error(chalk.red(`"${getAuthCommandName(command.kind)}" 存在未知选项 --${option}。`));
		console.error(chalk.dim(`使用 "${APP_NAME} --help" 或 "${getAuthCommandUsage(command.kind)}"。`));
		process.exitCode = 1;
		return true;
	}
	try {
...
export async function createSessionManager(
	...
	if (parsed.fork) {
		if (parsed.sessionId) {
			...
			if (existingTarget) {
				console.error(chalk.red(`已存在相同 ID 的会话：'${parsed.sessionId}'`));
				process.exit(1);
			}
		}
		...
		switch (resolved.type) {
			...
			case "not_found":
				console.error(chalk.red(`没有匹配 '${resolved.arg}' 的会话`));
				process.exit(1);
		}
	}
	...
	if (parsed.session) {
		...
		switch (resolved.type) {
			...
			case "global": {
				console.log(chalk.yellow(`会话属于其他项目：${resolved.cwd}`));
				const shouldFork = await promptConfirm("是否将该会话分叉到当前目录？");
				if (!shouldFork) {
	...
	if (parsed.sessionId) {
		...
		console.error(chalk.yellow(`警告：没有找到 ID 为 '${parsed.sessionId}' 的项目会话；将使用该 ID 创建新会话。`));
	}
...
export async function main(args: string[], options?: MainOptions) {
	...
	if (parsed.export) {
		...
		try {
			...
			result = await exportSessionHtmlFromFile(parsed.export, { outputPath });
		} catch (error: unknown) {
	...
	const createRuntime: CreateAgentSessionRuntimeFactory = async ({
		...
		if (parsed.apiKey) {
			if (!sessionOptions.model) {
				diagnostics.push({
					type: "error",
					message: "--api-key 需要通过 --model 或 --provider/--model 指定模型",
				});
			} else {
