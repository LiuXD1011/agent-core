...
Agent Core is a local coding agent. It runs with the permissions of the user account that starts it, and it treats files writable by that user as inside the same local trust boundary.
...
Agent Core considers a project to have resources that require trust when it finds any of these from the current working directory:

- `.agent-core/settings.json`
- `.agent-core/extensions`, `.agent-core/skills`, `.agent-core/prompts`, or `.agent-core/themes`
- `.agent-core/SYSTEM.md` or `.agent-core/APPEND_SYSTEM.md`
- project `.agents/skills` in the current directory or an ancestor directory

A bare `.agent-core` directory does not count as a project resource that requires trust.

When an interactive session starts in a project with resources that require trust and no saved decision for the current directory or a parent directory, pi follows `defaultProjectTrust` from global settings. The default value is `"ask"`, which asks whether to trust the project when UI is available. Saved decisions are stored by canonical directory in `~/.agent-core/agent/trust.json`, and the closest saved decision on the current or parent path applies before the global default.

Trusting a project allows pi to load project resources that require trust, including:

- `.agent-core/settings.json`
- `.agent-core` resources such as extensions, skills, prompt templates, themes, and system prompt files
...
- run the whole `agent-core` process inside a container/sandbox
- run host pi while routing built-in tool execution into a Gondolin micro-VM
- mount only the workspace paths the agent should access
- avoid mounting host `~/.agent-core/agent` unless the container should access host sessions, settings, and credentials
...
To report a security issue, follow the repository [Security Policy](https://github.com/LiuXD1011/agent-core/blob/main/SECURITY.md). Do not open a public issue for security-sensitive reports.
